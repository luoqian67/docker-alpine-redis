...
- `3.2.7`, `3.2`, `3`, `stable`, `latest` (2017-01-31, [Dockerfile](https://github.com/sickp/docker-alpine-redis/tree/master/versions/3.2.7/Dockerfile), [Release notes][release_notes_3_2])
- `3.2.6` (2016-12-06)
- `3.2.5` (2016-10-26)
- `3.2.4` (2016-09-26)
- `3.2.3` (2016-08-02)
...
```bash
$ docker run --rm sickp/alpine-redis:3.2.7 # redis-server /etc/redis.conf
               _._                                                  
          _.-``__ ''-._                                             
...
1:M 01 Feb 03:45:30.983 # WARNING: The TCP backlog setting of 511 cannot be enforced because /proc/sys/net/core/somaxconn is set to the lower value of 128.
1:M 01 Feb 03:45:30.983 # Server started, Redis version 3.2.7
1:M 01 Feb 03:45:30.983 # WARNING you have Transparent Huge Pages (THP) support enabled in your kernel. This will create latency and memory usage issues with Redis. To fix this issue run the command 'echo never > /sys/kernel/mm/transparent_hugepage/enabled' as root, and add it to your /etc/rc.local in order to retain the setting after a reboot. Redis must be restarted after THP is disabled.
1:M 01 Feb 03:45:30.983 * The server is now ready to accept connections on port 6379
```

Explore the image in a container shell:

```bash
$ docker run --rm -it sickp/alpine-redis:3.2.7 ash
/data #
```
...
```bash
$ docker run --rm sickp/alpine-redis:3.2.7 redis-server -v
Redis server v=3.2.7 sha=00000000:0 malloc=jemalloc-4.4.0 bits=64 build=c5eeece105c36294

$ docker run --rm sickp/alpine-redis:3.2.7 cat /etc/alpine-release
3.5.0
```
